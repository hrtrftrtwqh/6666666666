    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Minty address (e.g. MintyCoinfwYhBmGXcFP2Po1NpRUEiK8km2)"));
#endif
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if (!model->getOptionsModel() || !model->getOptionsModel()->getCoinControlFeatures())
      sendstatus = model->sendCoins(recipients, std::string());
    else
        sendstatus = model->sendCoins(recipients, std::string(), CoinControlDialog::coinControl);
...
void SendCoinsDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid Minty address"));
        }
        else
        {
