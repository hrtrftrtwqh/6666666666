...
unsigned int nStakeMaxAge = 60 * 60 * 30;
unsigned int nModifierInterval = 5 * 30;
...
const string strMessageMagic = "Minty Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    ...
    BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
    {
        ...
        if (!tx.msg.empty())
        {
          pwallet->adChanged(tx, pblock, fConnect);
        }
    }
}
...
static const int64_t nTargetTimespan = 13 * 60 * 60;  // 24 hours
//
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Minty", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        CTransaction txNew;
        txNew.nTime = 1440854743;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 4589440 << CBigNum(4782487) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("e2bad48465e2c993e264243377976be49ec36a29f677cfd5fd1aa4c4a739ed0f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x88, 0xf0, 0xb1, 0xe9 };
